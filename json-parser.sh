...
function output_entry() {
	...
	if [[ -n "$__FILTER" ]]; then 
		...
		eval "[[ '$path' =~ $regex ]] && add_out_buf '$path' '$value' "
	else 
        # if no filter is specified then add all key-value pair to output buffer
		add_out_buf "$path" "$value"
	fi 

    # if buf size is big enough,  then print it and clear it    
	(( ${#out_buf}>=out_buf_size )) && { echo -n "$out_buf" ; out_buf="" ; } 
}



function flush() {
    # check if there is something in the out buffer
	(( ${#out_buf}>=1 )) && { echo -n "$out_buf" ; out_buf="" ; } 
}


# read new line from stdin
function read_new_line() {
	...
	if [[ $i -ge $in_buf_len ]]; then  
		...
		IFS="${OLDIFS}"
		in_buf_len=${#in_buf}
		((chars_read=chars_read+in_buf_len+1,__LINE_ID++))
		[[ $stat -eq 0 ]] || { __EOF=1; ((chars_read=chars_read-1)) ; return $stat ; }	
	fi
}
...
function parse() {
	...
	clear_values
	__FILTER="$1"
	[[ -n "$__FILTER" ]] &&	build_filter_regex


	if [[ $cache_enabled -eq 1 ]]; then
		
		local nlen=${#name_cache[@]}
		local vlen=${#value_cache[@]}
		if [[ $vlen -ne $nlen ]]; then 
			clear_cache
			__CORRUPTED_CACHE=1
			local nlen=${#name_cache[@]}
			local vlen=${#value_cache[@]}
		...
		for	((x=0;x<nlen;x++)); do 
			...
			if [[ -n "$__FILTER" ]]; then 
				...
				eval "[[ '$name' =~ $regex ]] && add_out_buf '$name' '$value' "
			else 
				add_out_buf "$name" "$value"
			fi
			(( ${#out_buf}>=out_buf_size )) && { echo -n "$out_buf" ; out_buf="" ; } 
		done 
		...
		if ((__CORRUPTED_CACHE == 0 && nlen != 0 )); then 
			tell_errors
			return
		fi
	fi
